...
//JC
  ...
  {0x40,1,{0x10}},    // AVDD=2.5 VCI,AVEE=-3VCI       14
	...
	{0xA5,1,{0x17}},  // Gamma 52 / 203
	{0xA6,1,{0x09}},  // Gamma 80 / 175 
...
/*
    	...
    	//params->dsi.PLL_CLOCK = 214;//modify by xuefeng for shepin test
...
static void lcm_suspend(void)
{
	...
	*/
	//if (g_lcmSuspend != true)
      ...
      printk("linmingchuang add g_lcmSuspend = %d in suspend",g_lcmSuspend);
}
...
#if defined(BUILD_LK)
			  printf("ili9806e_txd_hd720_dsi lk -- ili9806e 0x%x , 0x%x , 0x%x, 0x%x ,%d\n", id_high, id_midd, id_low, id,lcm_adc);
#else
			  printk("ili9806e_txd_hd720_dsi kernel -- ili9806e 0x%x , 0x%x , 0x%x, 0x%x ,%d\n", id_high, id_midd, id_low, id,lcm_adc);
#endif
